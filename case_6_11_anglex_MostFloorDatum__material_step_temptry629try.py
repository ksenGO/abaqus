...
#step1
"""mdb.models['myModel'].HeatTransferStep(name='myStep1', previous='Initial', maxNumInc=1000, initialInc=0.1, minInc=0.001, maxInc=0.3,deltmx=200)
FRes = mdb.models['myModel'].fieldOutputRequests
FRes[FRes.keys()[0]].setValues(frequency=LAST_INCREMENT, variables=('NT','HFL'))
"""
moveTime=0.01
...
#fieldoutput
#fOR = mdb.models['myModel'].fieldOutputRequests.setValues(frequency=LAST_INCREMENT, variables=('HFL', 'NT'))
...
flux_face_region = regionToolset.Region(side2faces= flux_face)
mdb.models['myModel'].SurfaceHeatFlux(name='Heat Flux',createStepName ='myStep1',region = flux_face_region,magnitude=5000.0)
"""
    ...
    for y in range(1,im+1,1):#y*10im
        for z in range(1,mm+1,1):#z*m
          ...
          #  tempall = mdb.models['myModel'].steps['myStep1'].fieldOutputRequests['NT11']
            center =mdb.rootAssmbly.parts['Part-1'].nodeSets['dian1']
