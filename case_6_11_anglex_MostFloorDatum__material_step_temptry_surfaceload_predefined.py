...
p1.SectionAssignment(region = allset,sectionName = 'Section-Lv',offset =0.0,offsetType = MIDDLE_SURFACE,offsetField='',thicknessAssignment= FROM_SECTION)
print ("material is ok")
#Assembly
a = mdb.models['myModel'].rootAssembly
heatModel = mdb.models['myModel']
allsetName = 'Set-all'
ipart = heatModel.rootAssembly.instances[name='Part-1-1']
allset = heatModel.rootAssembly.Set(cells= ipart.cells,name = allsetName)
p11 = a.Instance(name = 'Part-1',part = p1,dependent = ON)
#Create the step
import step
heatModel.HeatTransferStep(name ='Heating Step',previous='Initial',description ='Apply heat in this step',response = STEADY_STATE,amplitude =RAMP)
...
mdb.models['myModel'].SurfaceHeatFlux(name = 'heatload', createStepName = 'Heating Step', region=region,magnitude = 400)
...
#mdb.models['myModel'].historyOutputRequests.changeKey(fromName='H-Output-1',toName='Selected History Outputs')
#mdb.models['myModel'].HistoryOutputRequest
#mdb.models['myModel'].historyOutputRequests['H-Output-1'].setValues(variables=('NT','HFL'))
#step1 load
#import load

#mdb.models['myModel'].parts['Part-1'].surface
#mdb.models['myModel'].SurfaceHeatFlux(name='flux',createStepName = 'Heating Step',region=)
