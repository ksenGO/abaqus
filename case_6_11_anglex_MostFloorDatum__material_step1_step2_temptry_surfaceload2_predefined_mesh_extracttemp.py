...
import job
#Creat the job
mdb.Job(name ='HeatTransferJob1',model ='myModel',type = ANALYSIS)
mdb.jobs['HeatTransferJob1'].submit(consistencyChecking=OFF)
mdb.jobs['HeatTransferJob1'].waitForCompletion()
print ("step1 is ok")
'''
#extract temp
...
import os
import sys
myodb = openOdb(path=r'D:\abaqus\00\HeatTransferJob.odb')
...
for x in range(1,jm+1,1):#x*jm
    for y in range(1,im+1,1):#y*10im
        for z in range(1,mm+1,1):#z*m
            ...
            center1 =myodb.rootAssembly.instances['PART-1-1'].nodeSets['dian1']
